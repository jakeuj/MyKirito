...
namespace MyKirito
{
    public interface IMyKiritoService
    {
        ...
        Task<(BattleLog battleLog, HttpStatusCode statusCode, ErrorOutput errorOutput)> Challenge(long userLv,
            string userUid, string userNickName);

        Task<string> ReIncarnation(long freePoints);
        Task<UserListDto> GetUserByName(string nickName);
        Task<UserListDto> GetUserListByLevel(long level, long page = 1);
    }
}